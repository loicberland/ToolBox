            ...
            <div className="plan-summary-header">
              ...
              <div className="plan-actions">
                <div className="plan-action-group">
                  {!plan.deletedAt && (
                    <>
                      {plan.latestRun?.status === 'running' && <Button type="button" size="sm" onClick={() => onRun(plan.latestRun!.id)}>{messages.testSheet.plans.continue}</Button>}
                      {plan.latestRun && plan.latestRun.status !== 'running' && <Button type="button" size="sm" onClick={() => onRun(plan.latestRun!.id)}>{messages.testSheet.plans.open}</Button>}
                      <Button type="button" size="sm" variant="secondary" disabled={plan.sheetCount === 0} onClick={() => createRun(plan)}>{messages.testSheet.plans.newRunShort}</Button>
                    </>
                  )}
                  <Button type="button" size="sm" variant="secondary" onClick={() => openHistory(plan)}>{messages.testSheet.plans.history}</Button>
                </div>
                <div className="plan-action-group plan-secondary-actions">
                  {plan.deletedAt ? (
                    <>
                      <Button type="button" size="sm" onClick={async () => { await testSheetApi.restorePlan(plan.id); await load(); }}>{messages.common.restore}</Button>
                    ...
                    <>
                      ...
                      <Button type="button" size="sm" variant="danger" onClick={() => setPlanToDelete(plan)}>{messages.testSheet.plans.hide}</Button>
                    </>
                  )}
                </div>
              </div>
